...
uint16_t LTDC_Graphic::getTextColor()
{
...
uint16_t LTDC_Graphic::getBackColor()
{
	uint16_t TextColor, BackColor;
	LCD_GetColors(&TextColor,&BackColor);
	return BackColor;
...
void LTDC_Graphic::drawPixel(int16_t x, int16_t y)
{
  ...
  drawLine(x, y, 1, LCD_DIR_HORIZONTAL);
}

size_t LTDC_Graphic::write(uint8_t n)
{
	if((LCD_HEIGHT - Y_Pos) < LCD_GetFont()->Height*2)
	{
		clear(getBackColor());
	}
...
void LTDC_Graphic::displayChar(uint16_t Line, uint16_t Column, uint8_t Ascii)
{
	if(Ascii != '\r' && Ascii != '\n')
	{
		...
		else LCD_DisplayChar(Line, Column,Ascii);
	}
}
